...
void main_loop(area_matrix &grid, ServerConnection &connection) {
...
int main(int argc, char const *argv[])
{
    ...
    generate_cells_uniform(grid, n_cells, {10, 100}, {10, 100}, 13);

    ServerConnection connection(hostname, port);
    connection.start_connection();
    connection.send_config_data(SMAX, SMAX, n_cells);

    for (int i = 0; i < 5000; i++) {
        update(grid);
    }
    //main_loop(grid, connection);
